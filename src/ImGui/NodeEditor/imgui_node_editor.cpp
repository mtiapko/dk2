...
//------------------------------------------------------------------------------
# include "dk/ImGui/NodeEditor/imgui_node_editor_internal.h"
...
void ed::Log(const char* fmt [[maybe_unused]], ...)
{
# if defined(_DEBUG) && defined(_WIN32)
    va_list args;
    va_start(args, fmt);
...
{
    auto line = m_StartPin->GetClosestLine(m_EndPin);
    if (m_StartPin->m_Node != nullptr) {
        const auto& bounds = m_StartPin->m_Bounds;
        line.A.x = (m_StartPin->m_Kind == PinKind::Output ? bounds.Max : bounds.Min).x;
    }

    if (m_EndPin->m_Node != nullptr) {
        const auto& bounds = m_EndPin->m_Bounds;
        line.B.x = (m_EndPin->m_Kind == PinKind::Output ? bounds.Max : bounds.Min).x;
    }
...
void ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
    ...
    /* XXX: viewport clip fix */
    auto canvas_vp = m_Canvas.ViewRect();
    auto vp = ImGui::GetMainViewport();
    auto old_pos = vp->Pos;
    auto old_size = vp->Size;
    vp->Pos = canvas_vp.Min;
    vp->Size = canvas_vp.Max - canvas_vp.Min;
