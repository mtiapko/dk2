...
renderer core::s_active_renderer;

template<> renderer* core::active<renderer>() noexcept { return &s_active_renderer; }

status core::run() noexcept
{
	size_t count = 0;
	auto avr = std::chrono::microseconds::zero();
	auto& update_event_mgr = event_manager<update_event>::get();
	auto& render_event_mgr = event_manager<render_event>::get();
